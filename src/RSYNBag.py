"""
Description: This file contains a class that implements "RSYN Bagging" algorithm using scikit-learn.
RSYNBag is a binary class classification algorithm.
...
__version__ = "0.0.1"
#---------------------------------------------------------------    
class RSYNBag(object):
    #initializer
    def __init__(self,n_estimator=10, estimator=DecisionTreeClassifier(), n_neighbors = 5):
    ...
    def __make_bootstraps(self,data):
        ...
        for b in range(self.n_estimator):
            if b % 2 == 0 and min(counts)/max(counts) < 0.8 and min(counts)>5:
                ...
                ada = ADASYN(random_state=42, sampling_strategy="auto", n_neighbors=self.n_neighbors)
